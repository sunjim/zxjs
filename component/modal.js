define(['zxcore'], function (zxcore) {
    return function (options, userCallback) {
        let opt = $.extend({
            ...
            footer: '',//底部
            id: 'hdMessage',//模态框id
            width: zxcore.isMobile() ? '95%' : 600,//宽度
            class: '',//样式
        ...
        modalObj.html(html);
        if (opt.width) {
            modalObj.find('.modal-dialog').css({'max-width': opt.width});
        }
        ...
        // if(typeof opt.events['keydown'] == 'function'){
        //     console.log(345);
        // }
        //点击搜索按钮事件
        if (typeof opt.events['search'] == 'function') {
            modalObj.find('.search', modalObj).on('click', function () {
                options.events['search'](modalObj);
                //console.log(1)
                //隐藏模态框
               // modalObj.modal('hide');
            });
